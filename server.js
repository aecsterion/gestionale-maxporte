...
<div id="modal-invia-preventivo" class="form-modal-overlay">
  <div class="form-modal" style="max-width:600px">
    <div class="form-modal-head">
      ...
      <button class="form-close" onclick="closeForm('modal-invia-preventivo')">×</button>
    </div>
    <div class="form-modal-body">
      <div class="form-field">
...
// ── INVIO PREVENTIVO ─────────────────────────────────────────────────────────
let _invioPayload = null;
let _invioDocId = null;
let _invioNumero = null;

async function apriModalInvioPreventivo(docId) {
  // Carica dati preventivo e anagrafica
  const {data: doc} = await sb.from('preventivi')
    .select('*, anagrafiche(*)')
    .eq('id', docId).single();
  if (!doc) { toast('Preventivo non trovato', 'err'); return; }

  const an = doc.anagrafiche || {};
  const numero = doc.numero || 'preventivo';
  const emailCliente = an.email_principale || an.email || '';
...
info@maxporte.it – www.maxporte.it`;
  ...
  // Costruisci payload PDF (stesso di scaricaPreventivo)
  _invioPayload = await buildPreventivoPayload(docId);

  const modal = ensureModalInBody('modal-invia-preventivo');
  modal.classList.add('open');
}

async function confermaInvioPreventivo() {
  const emailTo = document.getElementById('invia-email-to').value.trim();
  const oggetto = document.getElementById('invia-oggetto').value.trim();
  const testo = document.getElementById('invia-testo').value.trim();

  if (!emailTo || !oggetto || !testo) {
    toast('Compila tutti i campi obbligatori', 'err'); return;
  }
  if (!_invioPayload) {
    toast('Errore: payload PDF mancante', 'err'); return;
  }

  const btn = document.getElementById('btn-invia-conferma');
  btn.disabled = true;
  btn.textContent = '⏳ Invio in corso...';

  try {
    const resp = await fetch('/invia-preventivo', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        oggetto,
        testo,
        numero_preventivo: _invioNumero,
        preventivo_id: _invioDocId
      })
    });
    const result = await resp.json();
    if (result.ok) {
      // Aggiorna stato preventivo nel DB
      await sb.from('preventivi').update({
        stato: 'inviato',
      ...
      toast('Errore invio: ' + result.error, 'err');
    }
  } catch(e) {
...
async function renderPreventivoDetail(id){
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn btn-sm btn-red" onclick="apriModalInvioPreventivo('\${id}')">📧 Invia</button>
...
async function buildPreventivoPayload(id) {
  const {data:doc} = await sb.from('preventivi')
    .select('*, anagrafiche(*), agenti:agente_id(nome,cognome)')
    .eq('id',id).single();
  if(!doc) return null;
  const {data:righe} = await sb.from('righe_preventivo')
    .select('*').eq('preventivo_id',id).order('riga_numero',{ascending:true});
  const an = doc.anagrafiche || {};
  const ag = doc.agenti || {};
  const sc1 = doc.sconto1 || 0;
  return {
    documento: {
      tipo: 'PREVENTIVO',
      numero: doc.numero || '',
    ...
    righe: (righe||[]).map((r,i) => ({
      ...
      totale_riga_netto: (() => {
        ...
        return tot ? Math.round(tot * (1-sc1/100) * 100)/100 : 0;
      })(),
      sconto: sc1,
  ...
  if (req.method === 'POST' && req.url === '/invia-preventivo') {
    ...
    req.on('end', function() {
      try {
        ...
        const { payload, email_to, email_cc, oggetto, testo, numero_preventivo, preventivo_id } = data;
        generaPDF(payload, function(err, pdfData) {
          if (err) {
            res.writeHead(500, {'Content-Type':'application/json'});
            res.end(JSON.stringify({ok:false, error:'Generazione PDF: ' + err.message}));
            return;
          }
          ...
          transporter.sendMail(mailOptions, function(err2, info) {
            if (err2) {
              ...
              res.writeHead(500, {'Content-Type':'application/json'});
              res.end(JSON.stringify({ok:false, error:'Invio email: ' + err2.message}));
            } else {
